...
- [x] [BSOD-131] T003 Install all runtime and dev dependencies per research.md: `react@19.3`, `react-dom@19.3`, `react-router@8`, `dexie@4.4`, `dexie-react-hooks`, `recharts@3.10`, `zod@4.6`, `vite@8.3`, `vite-plugin-pwa@1.3`, `typescript@6.0`, `vitest@4.1`, `@testing-library/react`, `@testing-library/jest-dom`, `msw@2.15`, `@playwright/test@1.63`, `eslint@10.11`, `eslint-plugin-boundaries`, `prettier` — commits `package-lock.json`
...
- [x] [BSOD-351] T131 [P] [US1] e2e-first-landing-settings-credentials-panel red→green — write the Playwright spec in `tests/e2e/landing-credentials.spec.ts` that drives the production Docker build through the first-run walk (sibling BSOD-350 spec) and then through the post-first-run `/settings` route's `SettingsCredentialsPanel` (T045) — asserting the masked-token display (T044), Retest, Replace, and Switch-mode actions all function against the same MSW handlers `tests/integration/credentials/settings-panel.test.tsx` (BSOD-165 / T037) reuses; confirm it fails for the intended reason; then mount `SettingsCredentialsPanel` from a new `SettingsRoute` in `src/app/router.tsx` (as a child of `RouteGuard` so the gate contract stays intact) and add a `data-testid="nav-settings"` `<Link>` to the post-first-run placeholder so the e2e spec can navigate to it, so the test passes. **Sequential after T128 + BSOD-350 (T130)** — depends on the first-run composition landing and the placeholder being reachable.
- [x] [BSOD-352] T132 [P] e2e landing-shell coverage — add `tests/e2e/landing-shell.spec.ts` covering app-shell navigation chrome, the currently reachable first-run loading/empty/error surfaces through delayed/empty/500 API overrides, and unauthenticated route-guard gating on protected routes; note: the shared `LoadingState`/`EmptyState`/`ErrorState` primitives are not mounted on any live route yet, so this spec currently asserts the live first-run proxy states rather than those shared primitives directly. Reuse the existing Asana fixture shapes and keep this slice separate from the Stage-4 first-run and landing specs.

...
**Primary Dependencies**: React 19.3 (UI), Vite 8.3 + `vite-plugin-pwa` 1.3 /Workbox (build, service worker, installable PWA — first vite-plugin-pwa release with a Vite 8 peer range), Dexie.js 4.4 (typed, versioned IndexedDB access and schema migrations), Recharts 3.10 (SVG charting with tabular-alternative-friendly data model), React Router 8 (view navigation), Zod 4.6 (runtime validation of Asana API responses so malformed/missing fields are flagged per FR-081/FR-082 instead of silently coerced), MSW 2.15 — Mock Service Worker (deterministic Asana API fixtures for dev and tests per NFR-005).

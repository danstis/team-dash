...
**Primary Dependencies**: React 19.2 (UI), Vite 8.2 + `vite-plugin-pwa` 1.3 /Workbox (build, service worker, installable PWA — first vite-plugin-pwa release with a Vite 8 peer range), Dexie.js 4.4 (typed, versioned IndexedDB access and schema migrations), Recharts 3.10 (SVG charting with tabular-alternative-friendly data model), React Router 8 (view navigation), Zod 4.5 (runtime validation of Asana API responses so malformed/missing fields are flagged per FR-081/FR-082 instead of silently coerced), MSW 2.15 — Mock Service Worker (deterministic Asana API fixtures for dev and tests per NFR-005).
...
**Testing**: Vitest 4.1 + React Testing Library (unit and component tests, domain/metric logic isolated from React per Principle VI), Vitest + MSW fixture suites (Asana-client and storage "contract" tests), Playwright 1.63 (browser-based smoke tests: offline mode, PWA install/service-worker behaviour, credential flows) — all deterministic, none requiring a live Asana workspace or real token (Principle III, NFR-005).
...
| III | Test-First & Mandatory Automation | Testing stack (Vitest, RTL, MSW, Playwright) is fixture/mock-only — no live token or workspace required anywhere, satisfying NFR-005. Red/Green/Refactor is enforced as a single task deliverable: each behaviour-changing task commits a failing test, then the implementation that makes it green; CI MUST be green at every task close, and cross-task stubs that exist only to "unblock CI" between a red test's merge and its matching implementation's merge are forbidden. CI gate composition is captured in research.md. | PASS |

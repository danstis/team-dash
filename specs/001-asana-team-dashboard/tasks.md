...
- [x] [BSOD-296] T014b [P] Eliminate the `CredentialsProvider` / `WorkspaceProvider` / `FirstRunGateProbe` `useEffect`-ordering race surfaced by the BSOD-295 gate (`flushSync` the credentials + workspace state transitions; see `docs/architecture-decisions/0001-credentials-flush-sync.md`) so `npm run test:integration` is non-flaky (verified 55/55 across default + shuffled + non-concurrent-shuffled orderings on the BSOD-295 branch)

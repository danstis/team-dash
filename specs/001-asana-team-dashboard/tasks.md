...
- [x] [BSOD-352] T132 [P] e2e landing-shell coverage — add `tests/e2e/landing-shell.spec.ts` covering app-shell navigation chrome, LoadingState/EmptyState/ErrorState reachability through delayed/empty/500 API overrides, and unauthenticated route-guard redirection to `/`; reuse the existing Asana fixture shapes and keep this slice separate from the Stage-4 first-run and landing specs.

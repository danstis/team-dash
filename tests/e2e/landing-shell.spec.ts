...
import { smallDataset, smallDatasetWorkspaceGid } from "../../fixtures/asana/small-dataset/data";
...
test.describe("landing shell navigation, view states, and route guard", () => {
  test("renders app-shell navigation chrome after first-run completes", async ({ page }) => {
    await mockAsanaApi(page);
    await openFirstRun(page);
    await validateToken(page);
    await page.getByRole("radio", { name: /session[- ]only/i }).click();
    await page.getByRole("combobox", { name: /workspace/i }).selectOption(smallDatasetWorkspaceGid);
    await page.getByRole("button", { name: /select workspace/i }).click();

    await expect(page.getByRole("heading", { level: 1, name: /team dash/i })).toBeVisible();
    await expect(page.getByTestId("nav-settings")).toBeVisible();
    await expect(page.getByRole("link", { name: /settings/i })).toHaveAttribute("href", "/settings");
  });

  test("redirects unauthenticated visits to protected routes back to /", async ({ page }) => {
    await mockAsanaApi(page);
  ...
  test("reaches loading, empty, and error states through per-test API overrides", async ({ page }) => {
    await mockAsanaApi(page, {
      user: async (route) => {
    ...
    await page.getByLabel(/^token$/i).fill(FIXTURE_TOKEN);
    await page.getByRole("button", { name: /test token/i }).click();
    await expect(page.getByRole("button", { name: /testing token/i })).toBeVisible();
    ...
    await openFirstRun(page);
    await validateToken(page);
    await expect(page.getByText(/no accessible workspaces|no workspaces/i)).toBeVisible();

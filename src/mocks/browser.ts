 ...
 * activates lives at `dev/mockServiceWorker.js`. It is copied
 * verbatim from `node_modules/msw/lib/mockServiceWorker.js` (the
 * canonical MSW runtime file generated by `npx msw init dev/ --save`)
 * and is committed to the repository so the dev server serves it
 * without a build-time codegen dependency. It is deliberately kept
 * outside `public/` so Vite never copies it into the production
 * `dist/` bundle (BSOD-450); instead the `team-dash:dev-mock-service-worker`
 * plugin in `vite.config.ts` serves it at `/mockServiceWorker.js`
 * during `vite serve` only, so the `serviceWorker.url` default still
 * resolves in dev with no further configuration.
 *
 * Boundary
 * --------
 ...
 * registration fails (e.g. the `dev/mockServiceWorker.js` file is
 * missing or the dev-server middleware is not wired). The

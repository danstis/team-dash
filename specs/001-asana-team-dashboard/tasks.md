...
- [ ] T049 [US2] Refresh success outcome red→green — write the refresh-success integration test in `tests/integration/refresh/refresh-success.test.tsx` (progress then success state with completion timestamp); confirm it fails; then implement `RefreshButton`, `ProgressIndicator`, and the success / partial-failure rendering of `OutcomeBanner` in `src/features/refresh/RefreshControls.tsx` so the test passes. **Sequential: T049 must close before T051** (T051 extends `OutcomeBanner` with the failure-reason rendering and shares the same file).
